...
* Extract plain text from the html document using BeautifulSoup. The original html doc may contains many texts that are useless. For example, there are text in <stype> tag, which is useless and may influence our result. Therefore, we filter this useless texts to get more accurate result.
